 ...
 export default function dragsystem (scene) {
    ...
    if(scene.player.mana>5){
        ...
        if(unit.texture.key==='demon1'){              
            scene.newenemy=new Enemy(scene,pointer.worldX,pointer.worldY,"demon1","demon1_01",scene.University,2,200).setScale(1.5);
            scene.player.mana-=15;
            scene.manabar.cutManaBar(15);
            unit.x = originalX;
            unit.y = originalY;
            scene.enemies.add(scene.newenemy);}
